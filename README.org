...
If a kernel client sends code to be evaluated that causes the kernel to open a
file using =emacsclient=, if the opened file has the same =major-mode= as the
kernel language, then the file's buffer will also be associated with the
client. This behavior is most useful, for example, when using the =edit=
function in IJulia.
