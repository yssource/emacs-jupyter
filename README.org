...
There is also support for reading/writing the contents of directories the
notebook server has access to by using the normal Emacs file operations:
=find-file=, =delete-file=, and the like. This is done by adding new TRAMP file
name methods, =jpy= for HTTP connections and =jpys= for HTTPS connections. So
...
All file operations are supported.
